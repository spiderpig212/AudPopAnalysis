...
def plot_region_response_grid(grid_rdms, ordered_stims, regions, responses,
    ...
    for r, region in enumerate(regions):
        for t, target_region in enumerate(regions):
            for c, response in enumerate(responses):
                ax = axes[r][c]
                mats = grid_rdms.get((region, target_region, response), [])
                if mats:
                ...
                if c == 0:
                    ax.set_ylabel(f"{region}_{target_region}", fontsize=10, fontweight="bold")
        ...
        for session in uniqSessions:
            ...
            for brain_region in uniq_regions:
                ...
                for target_region in uniq_regions:
                    if target_region == brain_region:
                        continue  # TODO: Add neuron subsetting and then also include primary vs itself

                    target_mask = brain_session_array == target_region
                    target_resp_array = respArray[session_mask, :]
                    target_primary_array = target_resp_array[target_mask].T
                    ...
                    if primary_array.shape[0] == 0 or n_neurons < neuron_threshold:
                        continue
                    if target_primary_array.shape[0] == 0 or n_neurons < neuron_threshold:
                        continue
                    ...
                    cca = CCA(n_components=n_components_target)
                    primary_cca_array, target_cca_array = cca.fit_transform(primary_array, target_primary_array)

                    rdm = compute_crossnobis_rdm(
                        measurements=primary_cca_array,
                        stim_labels=session_stim_array,
                        session=session,
                        n_splits=n_splits,
                        reduce_dim=None,  # e.g. 10 or 0.9 for PCA path
                    ...
                    plot_rdm_heatmap(
                        ...
                        save_path=f"{file_path}/crossnobis_cca_rsa/rdm_"
                                  f"{stimulus}_{response_range}_{brain_region}_{target_region}_"
                                  f"{session}.png",
                    )

                    # Collect the reordered matrix for the group-average grid
                    grid_rdms.setdefault((brain_region, target_region, response_range), []).append(
                    ...
                    rsa_data.append(
                        {
                            ...
                            "target_brain_region": target_region,
                            "n_neurons": n_neurons,
            ...
            if len(rsa_data) == 0:
                print(f"No RSA data for {stimulus}/{response_range}/{brain_region}/{target_region} -> skipping")
                continue
            rsa_results_df = pd.DataFrame(rsa_data)
            out_name = (
                f"{file_path}/crossnobis_cca_rsa/"
            ...
            for region in uniq_regions:
                for target in uniq_regions:
                    subset = rsa_results_df[
                        (rsa_results_df["brain_region"] == region)
                        & (rsa_results_df["target_brain_region"] == target)
                    ]

                    plot_fisher_across_regions(
                        subset,
                        title=f"Fisher information | {stimulus} | {response_range}",
                        save_path=f"{file_path}/crossnobis_cca_rsa/fisher_"
                    ...
                    plot_region_response_grid(
                        ...
                        save_path=f"{file_path}/crossnobis_cca_rsa/"
